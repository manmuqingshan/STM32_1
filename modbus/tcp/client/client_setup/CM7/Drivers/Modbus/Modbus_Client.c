...
static err_t Modbus_Client_Init(struct tcp_pcb *tpcb, char *serverIP, uint16_t port);
...
static err_t Modbus_Client_Init(struct tcp_pcb *tpcb, char *serverIP, uint16_t port)
{
    ...
    return tcp_connect(tpcb, &destIPADDR, port, tcp_client_connected);
}
...
#define TCP_MAX_RETRIES           5
	...
	if (isServerOK)
	{
		...
		if (!tcp_started)
		{
			...
			if (Modbus_Client_Init(ClientPCB, serverIP, port) == ERR_OK)
			{
				tcp_started = 1;
...
err_t TCP_SendRequest(struct tcp_pcb *tpcb, uint8_t *txBuf, uint16_t length)
{
    ...
    err = tcp_write(tpcb, txBuf, length, TCP_WRITE_FLAG_COPY);
    if(err != ERR_OK)
        return err;
